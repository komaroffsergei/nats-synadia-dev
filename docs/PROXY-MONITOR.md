...
Заголовок обычной сессии берётся из первого содержательного поручения. Известные служебные блоки в пользовательском вводе (`environment_context`, `recommended_plugins`, `in-app-browser-context`, `skills_instructions`, `permissions`) свёрнуты, но раскрываются целиком с исходной разметкой. Смешанное сообщение сохраняет видимый текст поручения и отдельные сворачиваемые блоки. Флаг `context` в событии означает повторно переданную историю и не используется для скрытия поручений. Сохранённые тексты не переписываются: при обновлении пересчитываются только заголовки и вид активности. Прежние ограничения очистки секретов и хранения system/developer инструкций сохраняются.
