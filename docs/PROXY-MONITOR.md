...
Название чата задаётся владельцем вручную в Synadia. Оно хранится отдельно в `session_titles`, не заменяется новыми событиями и не переименовывает чат в Codex. Подключения Desktop/App Server и синхронизации локальных названий нет. Пока имя не задано, заголовок — «Чат без названия» (для вспомогательного запроса — «Создание названия чата»); первое поручение и предложение модели остаются явно подписанными подсказками. Ручное имя используется в поиске, списке, деталях и сравнении.
