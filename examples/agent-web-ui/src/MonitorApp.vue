...
const kindLabel=(k:string)=>({reasoning:'Размышления',message:'Сообщение',tool_call:'Вызов инструмента',tool_result:'Результат инструмента'}[k] || k);
...
<template>
  <div class="monitor">
    ...
    <template v-if="privateMode">
      ...
      <section class="metrics"><div><span>Сессии</span><strong>{{format(sessions.length)}}</strong></div><div><span>Активные запросы</span><strong class="green">{{format(hasActivity)}}</strong></div><div><span>Токены · новая история</span><strong>{{format(usage?.total)}}</strong></div><div><span>Полнота учёта</span><strong class="small-stat">{{usage?qualityLabel(usage.quality):'Ожидание данных'}}</strong></div></section>
      <main v-if="tab==='sessions'||tab==='traffic'" class="workspace" :class="{'has-selection':selectedId}">
        <aside class="session-list">
          ...
          <button v-for="s in shown" :key="s.id" class="session-card" :class="{selected:s.id===selectedId}" @click="select(s.id)">
            ...
            <strong>{{s.title||'Сессия без названия'}}</strong><span class="model mono">{{s.model||'Модель пока неизвестна'}}</span>
        ...
        <section class="detail">
          ...
          <template v-else>
            <div class="detail-heading"><button class="mobile-back" @click="selectedId='';selected=null">← Сессии</button><div><span class="eyebrow">{{selected.model||'CODEX'}} · {{stateLabel(selected.status)}}</span><h2>{{title}}</h2></div><button class="primary" @click="openPreview">Опубликовать ↗</button></div>
            ...
            <template v-if="tab==='sessions'">
              ...
              <button v-if="selected?.hasOlder&&olderAvailable" class="secondary" :disabled="loadingOlder" @click="loadOlder">{{loadingOlder?'Загружаем…':'Показать более ранние события'}}</button><div class="timeline">
                <details v-for="item in visibleItems" :key="item.itemId" :class="['timeline-item',item.kind]" :open="true">
                  <summary><span class="item-symbol">{{item.kind==='reasoning'?'✦':item.kind.startsWith('tool')?'⌘':'↗'}}</span><strong>{{item.role==='user'?'Поручение':kindLabel(item.kind)}}</strong><span v-if="item.name" class="mono tool-name">{{item.name}}</span><time>{{time(item.at)}}</time></summary>
                  <div class="item-body"><pre>{{item.text||'Ожидаем содержимое…'}}</pre><span v-if="!item.complete" class="streaming-label">● Поступает текст</span><p v-if="item.truncated" class="notice">Размер элемента превысил предел хранения 2 MiB; текст неполный.</p></div>
                </details>
              </div>
