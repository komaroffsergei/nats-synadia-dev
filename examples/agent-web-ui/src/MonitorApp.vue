...
const ownerBroadcasts=ref<any[]>([]),repeatBusy=ref(false);
const sessionBroadcasts=computed(()=>ownerBroadcasts.value.filter(b=>b.sessionId===selectedId.value && b.mode==='live'));
const repeats=(id:string)=>ownerBroadcasts.value.some(b=>b.sessionId===id && b.visible && b.repeatWhenIdle);
async function setRepeat(row:any,checked:boolean){
  repeatBusy.value=true;
  try{await api(`/api/v1/monitor/broadcasts/${row.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({repeatWhenIdle:checked})});await refresh();}
  catch(e){error.value=(e as Error).message;}finally{repeatBusy.value=false;}
}
...
const stateLabel=(s:string)=>({streaming:'Активна',waiting:'Ожидание данных',quiet:'Нет новых событий',cancelled:'Остановлена',completed:'Ответ завершён',failed:'Ошибка',incomplete:'Поток прерван',connecting:'Подключение',connected:'Подключено',reconnecting:'Переподключение',revoked:'Доступ закрыт'}[s] || s);
const qualityLabel=(q:string)=>q==='complete'?'Полные данные':q==='conflict'?'Есть расхождения':'Неполные данные';
const shown=computed(()=>sessions.value.filter(s=>`${s.title} ${s.model} ${s.sourceUser||s.sourceUserId} ${s.sourceConfig||''}`.toLowerCase().includes(search.value.toLowerCase())));
...
async function refresh() {
  ...
  try {
    if(privateMode) {
      const [s,u,c,b]=await Promise.all([api('/api/v1/monitor/sessions'),api('/api/v1/monitor/usage'),api('/api/v1/monitor/connections'),api('/api/v1/monitor/broadcasts')]);
      ownerBroadcasts.value=b;
      ...
      const row=current.get(selectedId.value);
      if(row && selected.value)selected.value={...selected.value,status:row.status,sourceUser:row.sourceUser,sourceConfig:row.sourceConfig};
...
async function editTitle() {
  titleDraft.value=selected.value?.customTitle || selected.value?.title?.slice(0,160) || '';titleError.value='';editingTitle.value=true;
...
const broadcastRepeat=ref(false);
async function openBroadcast(repeat=false) {
  broadcastRepeat.value=repeat;
  showPublish.value=false;
  tab.value='broadcasts';
  await nextTick();
  document.getElementById('broadcast-session')?.focus();
}
onMounted(async()=>{
  await refresh();
...
<template>
  <div class="monitor">
    <header class="connection-bar">
      <a class="brand" :href="privateMode?'/console/':'/'"><span class="brand-mark">//</span>Codex Monitor <small>NATS JETSTREAM</small></a>
    ...
    <template v-if="privateMode">
      ...
      <main v-if="tab==='sessions'||tab==='traffic'" class="workspace" :class="{'has-selection':selectedId}">
        <aside class="session-list">
          ...
          <label class="search"><span class="sr-only">Поиск сессий</span><input v-model="search" placeholder="Поиск по задаче, модели или источнику" /></label>
          ...
          <button v-for="s in shown" :key="s.id" class="session-card" :class="{selected:s.id===selectedId}" @click="select(s.id)">
            <span class="card-top"><span class="badge" :class="s.status">{{stateLabel(s.status)}}</span><span v-if="repeats(s.id)" class="shared-label">↻ Автоповтор</span><span v-else-if="s.shared" class="shared-label">Есть временная ссылка</span></span>
            <span v-if="s.activity==='title_generation'" class="service-label">Служебный запрос · название чата</span>
            <strong :title="s.title">{{s.title}}</strong><span class="source-label">Пользователь: {{s.sourceUser || ('ID ' + s.sourceUserId?.slice(0,8))}}<br>Конфигурация: {{s.sourceConfig || 'не передана proxy'}}</span><span class="model mono">{{s.model||'Модель пока неизвестна'}}</span>
        ...
        <section class="detail">
          ...
          <template v-else>
            <div class="detail-heading"><button class="mobile-back" @click="selectedId='';selected=null">← Сессии</button><div><span class="eyebrow" title="Статус определяется запросами proxy. Завершение ответа не означает завершение задачи. После 90 секунд без событий активный запрос отмечается как ожидание данных.">{{selected.model||'CODEX'}} · {{stateLabel(selected.status)}}</span><h2>{{title}}</h2><p class="source-label">Пользователь: {{selected.sourceUser || ('ID ' + selected.sourceUserId?.slice(0,8))}} · Конфигурация последнего подключения: {{selected.sourceConfig || 'не передана proxy'}}</p><button v-if="!editingTitle" class="title-edit-link" @click="editTitle">{{selected.customTitle?'Изменить название чата':'Задать название чата'}} ✎</button></div><div class="publication-actions"><button class="primary" @click="openBroadcast(false)">Эфир на сайте ↗</button><button class="secondary" @click="openBroadcast(true)">Эфир + автоповтор ↻</button><button class="secondary" @click="openPreview">Временная ссылка</button></div></div>
            <form v-if="editingTitle" class="title-editor" @submit.prevent="saveTitle()" @keydown.esc.prevent="editingTitle=false">
              ...
              <p id="title-help">Название задаётся вручную в Codex Monitor. Чат в Codex не переименовывается. В уже созданных публикациях сохраняется прежнее имя.</p>
            ...
            <div v-if="sessionBroadcasts.length" class="session-broadcasts"><label v-for="row in sessionBroadcasts" :key="row.id"><input type="checkbox" :checked="row.repeatWhenIdle" :disabled="repeatBusy" @change="setRepeat(row,($event.target as HTMLInputElement).checked)" /> Повторять эфир «{{row.title}}» после запроса <small>{{row.visible ? 'Опубликован' : 'Скрыт'}}</small><span v-if="row.replayProblem" class="warning-text">Запись превысила лимит или недоступна — измените фрагмент в разделе «Эфир сайта».</span></label></div>
            ...
            <template v-if="tab==='sessions'">
              ...
              <section v-if="shares.length" class="shares"><h3>Временные ссылки этой сессии</h3><p class="session-explanation">Ссылки открывают сессию на agents.komaroff-dev.ru. Публикацией на главной управляет раздел «Эфир сайта».</p><div v-for="s in shares" :key="s.id"><span>{{s.revoked_at?'Доступ отозван':Date.parse(s.expires_at)<Date.now()?'Срок истёк':`Доступна до ${date(s.expires_at)}`}}</span><button v-if="!s.revoked_at&&Date.parse(s.expires_at)>Date.now()" class="danger-link" @click="revoke(s.id)">Отозвать</button></div></section>
            </template>
            <template v-else>
      ...
      <BroadcastManager v-else-if="tab==='broadcasts'" :sessions="sessions" :selected-id="selectedId" :repeat-idle="broadcastRepeat" />
    ...
    <main v-else class="public-page">
      ...
      <footer><a href="https://komaroff-dev.ru/">← Портфолио</a><span>Codex Monitor · NATS · Codex proxy</span></footer>
    </main>
    <div v-if="showPublish" class="modal-backdrop" @click.self="showPublish=false" @keydown.esc="showPublish=false"><section ref="dialog" @keydown="trapFocus" role="dialog" aria-modal="true" aria-labelledby="publish-heading" class="publish-dialog"><div class="panel-heading"><h2 id="publish-heading">Временная ссылка на сессию</h2><button aria-label="Закрыть" @click="showPublish=false">✕</button></div><p>Ссылка откроет сессию на agents.komaroff-dev.ru на выбранный срок. На главную портфолио она не добавляется. Для главной используйте «Эфир на сайте». Проверьте текст перед созданием ссылки.</p><div class="publish-options"><label>Начало<select v-model="shareFrom"><option value="history">Доступная история и новые события</option><option value="now">Только новые события</option></select></label><label>Срок, часов<input v-model="shareHours" type="number" min="1" max="168" /></label></div><p class="publication-title"><strong>Название по ссылке:</strong> {{preview?.title}}</p><div class="preview-content"><template v-if="shareFrom==='history'"><MonitorTimelineItem v-for="(item,i) in preview?.items" :key="i" :item="item" :activity="selected?.activity" /><p v-if="!preview?.items?.length">Текст пока не поступил.</p></template><p v-else>Будут видны только события после публикации.</p></div><div v-if="shareLink" class="share-result"><label>Временная ссылка<input readonly :value="shareLink" @focus="($event.target as HTMLInputElement).select()" /></label><a :href="shareLink" target="_blank" rel="noreferrer">Открыть сессию ↗</a><button class="secondary" @click="openBroadcast(false)">Настроить эфир сайта ↗</button></div><div v-else class="modal-actions"><button class="secondary" @click="showPublish=false">Отмена</button><button class="primary" :disabled="publishing" @click="publish">{{publishing?'Создаём ссылку…':'Создать временную ссылку'}}</button></div></section></div>
  </div>
</template>

<style scoped>
.publication-actions{display:flex;flex-direction:column;gap:8px;flex-shrink:0}
@media(max-width:650px){.detail-heading{flex-wrap:wrap}.detail-heading>div:not(.publication-actions){min-width:0}.publication-actions{width:100%;flex-direction:row;flex-wrap:wrap}.publication-actions button{flex:1;white-space:normal}}
...
.source-label{font-size:11px;line-height:1.6;color:var(--text-muted);overflow-wrap:anywhere}.publication-actions{flex-wrap:wrap}.badge.completed{color:var(--info)}.badge.waiting{color:var(--warning)}.session-broadcasts{display:grid;gap:8px;margin-top:14px}.session-broadcasts label{display:flex;gap:8px;align-items:center;flex-wrap:wrap;font-size:12px;line-height:1.6}.session-broadcasts input{width:18px;height:18px}.session-broadcasts small{color:var(--text-muted)}
</style>
