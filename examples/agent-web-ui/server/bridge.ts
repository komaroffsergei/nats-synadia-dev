...
export class Bridge {
  ...
  private async handleBasicGroupCreate(
    id: string,
    controllerInstanceId: string,
    spec: unknown,
  ): Promise<void> {
    ...
    try {
      ...
      if (errHeader) {
        this.sendError(
          id,
          errHeader,
          rep.headers?.get("Nats-Service-Error") ?? "basic group create error",
        );
        return;
      }
  ...
  private async handleBasicGroupStop(
    id: string,
    controllerInstanceId: string,
    sessionId: string,
  ): Promise<void> {
    ...
    try {
      ...
      if (errHeader) {
        this.sendError(
          id,
          errHeader,
          rep.headers?.get("Nats-Service-Error") ?? "basic group stop error",
        );
        return;
      }
      for (const [instanceId, agent] of this.agentsByInstanceId) {
  ...
  private async handleBasicGroupList(
    id: string,
    controllerInstanceId: string,
  ): Promise<void> {
    ...
    try {
      ...
      if (errHeader) {
        this.sendError(
          id,
          errHeader,
          rep.headers?.get("Nats-Service-Error") ?? "basic group list error",
        );
        return;
      }
  ...
  private resolveBasicControllerSubject(
    ...
    if (!agent) {
      this.sendError(
        id,
        "agent_not_found",
        `no basic controller with instance id ${controllerInstanceId} in last discovery result — click Refresh`,
      );
      return null;
    }
    if (agent.agent !== "basic") {
      this.sendError(
        id,
        "wrong_agent_token",
        `instance ${controllerInstanceId} is agent="${agent.agent}", expected "basic"`,
      );
      return null;
    }
    if (agent.metadata["role"] !== "controller") {
      this.sendError(
        id,
        "not_a_controller",
        `instance ${controllerInstanceId} is not a basic controller`,
      );
      return null;
    }
    const tokens = agent.promptEndpoint.subject.split(".");
